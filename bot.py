...
already_registered_messages = ['Вы уже в турнире, зачем обманывать?', 'Не нужно регистрироваться несколько раз',
                               'Парень, зачем тебе играть, купи себе мозги']
...
tournament_winner_messages = ['Победитель турнира: %s', 'Поздравляем, %s', 'Хорошая работа, %s']
...
set_emoji_football = ['Сетку не порвите', 'Скинь мяч!', 'Где вы Месси тут увидали?']
set_emoji_bowling = ['Покатаем шары']
...
football_names = ['футбиш', 'футбол', 'soccer', 'football', 'футбик', 'ногамяч']
bowling_names = ['боулинг', 'bowling', 'кегли']
coin_like_emoji = ["🏀", "⚽"]
    ...
    if currentTournament.is_started():
        ...
        if len(args) == 0:
            ...
            if arg in dart_names:
                ...
                update.message.reply_text(random.choice(set_emoji_football))
            elif arg in bowling_names:
                current_emoji = "🎳"
                update.message.reply_text(random.choice(set_emoji_bowling))
            else:
...
def get_text_stats(stats):
    ...
    for p in stats:
        if p[3] != 0:
            prepared_stat1.append([p[0], p[3] // 2, str(p[4] * 100 // p[3]) + '%', p[5] * 10 * 2 // p[3] / 10, p[4]])
        if p[6] != 0:
...
def my_help(bot, update):
    update.message.reply_text(
        ...
        '4) Можно указать вид состязания, командой с аргументом /emoji dice|dart|basketball|football. \n'
